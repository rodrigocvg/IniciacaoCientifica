                    ...
                    for umavez in frameBox: #roda apenas uma vez, pq so tem um
                        ...
                        for boxFOR in umavez.values():
                            ...
                            if(distanciaI<menorDist):
                                 ...
                                 if(menorDist<5):
                                    frameBox.append({"mesmocara"+str(frameI):menorDistObj})
                                    
                                 else:
                                    #print("kkdsadj")
                                    DicioClassId.append({"daMerge"+idObj:[{frameI:box}]})
    ...
    for(classid,score,box) in zip(classes,scores,boxes):
        ...
        if not classid+1 in Dicio:
                ...
                if not idObj in DicValor:
                        ...
                        Dicio[int(classid+1)].append({idObj:DicValor[idObj]})
        else:
                ...
                # print("box",box)
                # print("frameI",frameI)
                merge5(Dicio[classid+1],box,frameI,idObj) #ta puxando idObj errado
                #except:
                     #pass
        contDetec+= 1
